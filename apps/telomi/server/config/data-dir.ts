...
// Root of the snapshots `npm run upgrade` takes. Next to the data directory by default, so a
// copy-on-write clone stays on the same volume. Each installation keeps its snapshots, lock and
// upgrade marker in its own `<root>/<installationId>/` (`installationBackupDir()` in data-format.ts),
// so installations sharing a root never see each other's.
