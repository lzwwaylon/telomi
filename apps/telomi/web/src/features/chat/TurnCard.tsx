...
import type { ActivityItem, ActivityStatus, ResponseContent, TodoItem, TodoStatus } from '@/features/goals/data/types'
...
import { PAPER_THEME_CHANGE_EVENT, isCurrentPaperThemeDark } from '@/shared/lib/theme'
import { MarkdownView } from '@/shared/markdown/MarkdownView'
import { Spinner } from '@/shared/ui/loading-indicator'
import { Tooltip, TooltipTrigger, TooltipContent } from '@/shared/ui/tooltip'
import { computeLastChildSet, groupActivitiesByParent, isActivityGroup, formatDuration, formatTokens, deriveTurnPhase, shouldShowThinkingIndicator, type ActivityGroup, type AssistantTurn } from '@/features/chat/turn-utils'
import { ResponseFullscreenOverlay } from '@/app/overlays/ResponseFullscreenOverlay'
...
function formatToolDisplay(
  ...
  const { toolName, displayName, toolDisplayMeta } = activity
...
export function getPreviewText(
  ...
  if (firstTask?.toolInput?.description) {
    return `${firstTask.toolInput.description as string}${statusSuffix}`
  }

  // When complete, show summary (badge already shows count)
  if (isComplete || (!isStreaming && activities.length > 0)) {
    return `${i18n.t('turnCard.stepsCompleted')}${statusSuffix}`
  }

  // Live turn with nothing running: the model is deciding its next step
  if (isStreaming) return uiText("chat.turncard.thinking")
  return i18n.t('turnCard.starting')
}

function getLogicalStepCount(activities: ActivityItem[]): number {
  const tools = activities.filter(activity => activity.type === 'tool')
  if (tools.length === 0) return activities.length
  return new Set(tools.map(activity => activity.operationId ?? activity.id)).size
}
...
export function ActivityStatusIcon({
  ...
  const renderIcon = () => {
    ...
    switch (status) {
      ...
      case 'completed': {
        // Per-tool Lucide icon overrides.
        // for Edit/Write, then extends to the rest of the native tool table
        // (cf. server-core SessionManager.resolveToolDisplayMeta nativeToolNames)
        // plus telomi private tools (artifacts / javascript_repl / extract_document).
        // Bash with brand match has customIcon and is handled above.
        const iconCls = cn(SIZE_CONFIG.iconSize, "shrink-0 text-accent")
        switch (toolName) {
          case 'Edit':
          case 'edit':
            return <Pencil className={iconCls} />
          case 'Write':
          case 'write':
            return <FilePenLine className={iconCls} />
          case 'Read':
          case 'read':
          ...
          case 'Glob':
            return <FolderSearch className={iconCls} />
          case 'Task':
          case 'Agent':
          ...
          case 'TodoWrite':
            return <ListTodo className={iconCls} />
          case 'NotebookEdit':
            return <Notebook className={iconCls} />
          case 'KillShell':
            return <Square className={iconCls} />
          case 'TaskOutput':
            return <ScrollText className={iconCls} />
          case 'artifacts':
            return <FilePlus2 className={iconCls} />
          case 'javascript_repl':
      ...
      case 'recovered':
        return <CircleCheck className={cn(SIZE_CONFIG.iconSize, "shrink-0 text-[var(--warm-deep)]")} />
      case 'error':
...
/** Indents a nested row by its depth. */
...
const DETAILS_ROW_FOCUS = "rounded-[3px] focus:outline-none focus-visible:ring-1 focus-visible:ring-ring"

/** Makes a row that opens details a keyboard button, so the details dialog returns focus to the row on close. */
function detailsTrigger(onOpen: (() => void) | undefined): React.HTMLAttributes<HTMLDivElement> {
  if (!onOpen) return {}
  return {
    role: 'button',
    tabIndex: 0,
...
function DetailsArrow() {
  return (
    <span
      aria-hidden
      className="p-0.5 rounded-[3px] opacity-0 group-hover/row:opacity-100 group-focus-visible/row:opacity-100 transition-opacity shrink-0"
    >
...
export function ActivityRow({ activity, onOpenDetails, isLastChild, displayMode = 'detailed' }: ActivityRowProps) {
  ...
  if (activity.type === 'intermediate') {
    ...
    const displayContent = isThinking ? uiText("chat.turncard.thinking") : stripMarkdown(activity.content || '')
    ...
    return (
      <div className="flex items-stretch">
        ...
        <div
          ...
          {...detailsTrigger(onOpenDetails && isComplete ? onOpenDetails : undefined)}
        >
          {isThinking ? (
            <div className={cn(SIZE_CONFIG.iconSize, "flex items-center justify-center shrink-0")}>
              <Spinner className={SIZE_CONFIG.spinnerSize} />
          ...
          {onOpenDetails && isComplete && <DetailsArrow />}
        </div>
      </div>
    )
  }

  // Thinking blocks: render via collapsible ThinkingBlock so the user can
  // expand the actual reasoning text instead of seeing only a one-line
  // "Thinking" stub. While streaming the block force-expands; on completion
  // it snaps to collapsed.
    ...
    return (
      <div className="flex items-stretch">
        ...
        <div className="flex-1 min-w-0">
          <ThinkingBlock content={activity.content || ''} isStreaming={isRunning} />
        </div>
      </div>
    )
  }
  ...
  const inputSummary = formatToolInput(activity.toolInput, activity.toolName)
  const isComplete = activity.status === 'completed' || activity.status === 'recovered' || activity.status === 'error'
  ...
  return (
    <div className="flex items-stretch">
      ...
      >
        ...
        {/* Filename badge for Read tool */}
        {!isMcpOrApiTool && !isBackgrounded && activity.toolName === 'Read' && typeof activity.toolInput?.file_path === 'string' && (
          <span className="flex items-center gap-1.5 text-[10px] shrink-0">
            <span className="px-1.5 py-0.5 bg-background shadow-minimal rounded-[4px] text-[11px] text-foreground/70">
              {activity.toolInput.file_path.replaceAll('\\', '/').split('/').pop()}
            </span>
          </span>
        )}
        ...
        {!isMcpOrApiTool && activity.status === 'recovered' && (
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="shrink-0 rounded-[4px] bg-[color-mix(in_oklab,var(--warm)_10%,var(--background))] px-1.5 py-0.5 text-[10px] font-medium text-[var(--warm-deep)]">
					{uiText("chat.turncard.recovered")}
              </span>
            </TooltipTrigger>
            <TooltipContent side="top" className="max-w-[400px]">
				  {uiText("chat.turncard.thisAttemptFailedButALaterRunOfThe")}
            </TooltipContent>
          </Tooltip>
        )}
 ...
 */
function ActivityGroupRow({ group, expandedGroups, onExpandedGroupsChange, onOpenActivityDetails, animationIndex = 0 }: ActivityGroupRowProps) {
  ...
  }, [groupId, expandedGroups, onExpandedGroupsChange])
  ...
  const isComplete = group.parent.status === 'completed' || group.parent.status === 'recovered' || group.parent.status === 'error'
    ...
    >
      ...
      >
        ...
        <span className="shrink-0 px-1.5 py-0.5 rounded-[4px] bg-background shadow-minimal text-[10px] font-medium">
          {subagentType || uiText("chat.turncard.task")}
        </span>
        ...
        <span className={cn(
          ...
          {description || uiText("chat.turncard.task")}
        </span>
...
/**
 * ResponseCard - Unified card component for AI responses
 *
...
function ResponseCard({
  ...
  // Only apply edge fade when the content actually overflows MAX_HEIGHT —
  // otherwise short messages get their first/last line silently faded out.
  const [hasOverflow, setHasOverflow] = useState(false)
  const contentRef = useRef<HTMLDivElement>(null)
  const contentLayerRef = useRef<HTMLDivElement>(null)

  // Detect paper dark mode and listen for theme changes
  useEffect(() => {
    const checkDarkMode = () => {
      setIsDarkMode(isCurrentPaperThemeDark())
    }
    checkDarkMode()

    window.addEventListener(PAPER_THEME_CHANGE_EVENT, checkDarkMode)
    // Observe data attr changes on documentElement for theme switches
    const observer = new MutationObserver(checkDarkMode)
    observer.observe(document.documentElement, { attributes: true, attributeFilter: ['data-paper-dark', 'data-paper-theme'] })
    return () => {
      window.removeEventListener(PAPER_THEME_CHANGE_EVENT, checkDarkMode)
      observer.disconnect()
    }
  }, [])
  ...
  useEffect(() => {
    ...
    const inner = contentLayerRef.current
    if (!outer || !inner) return
    const measure = () => {
      setHasOverflow(outer.scrollHeight > outer.clientHeight + 1)
    }
    measure()
      ...
      <div className="bg-background shadow-minimal rounded-[8px] overflow-hidden relative group">
        ...
        <div
          ...
          style={{
            ...
            // Subtle fade at top and bottom edges (16px) - only in dark mode and when content actually overflows
            ...(isDarkMode && hasOverflow && {
  ...
  return (
    <div data-streaming-active className="bg-background shadow-minimal rounded-[8px] overflow-hidden group">
      ...
      <div className={cn("px-4 py-2 border-t border-border/30 flex items-center bg-muted/20", SIZE_CONFIG.fontSize)}>
        <div className="flex items-center gap-2 text-muted-foreground">
          <Spinner className={SIZE_CONFIG.spinnerSize} />
				<span>{uiText("chat.turncard.streaming")}</span>
        </div>
      </div>
    </div>
  )
 ...
 */
/** "Thinking…" line shown from the moment a turn is sent until its first activity streams in. */
export function ThinkingPlaceholder() {
  return (
    <div className={cn("flex items-center gap-2 px-3 py-1.5 text-muted-foreground", SIZE_CONFIG.fontSize)}>
      <Spinner className={SIZE_CONFIG.spinnerSize} />
      <span>{uiText("chat.turncard.thinking")}</span>
    </div>
  )
}
...
export const TurnCard = React.memo(function TurnCard({
  ...
  const logicalStepCount = useMemo(
    () => getLogicalStepCount(activities),
    [activities]
  )
  ...
  // A live turn always renders: its first thinking block arrives empty and fills in.
  const hasNoMeaningfulWork = isComplete
    ...
    && activities.every(a => {
      ...
      // Intermediate and thinking activities must have no meaningful content
      if (a.type === 'intermediate' || a.type === 'thinking') return !a.content?.trim()
  ...
  return (
    <div className="space-y-1">
        ...
        <div className="group select-none" data-search-exclude="true">
          ...
          >
            <button
              type="button"
              onClick={toggleExpanded}
              className="flex min-w-0 flex-1 items-center gap-2 py-1.5 pl-2.5 text-left focus:outline-none focus-visible:ring-1 focus-visible:ring-ring"
            >
            ...
            <span className="relative flex-1 min-w-0 h-5 flex items-center">
              <AnimatePresence initial={false}>
                <motion.span
                  key={isStreaming ? "live" : previewText}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
            ...
            <div className="shrink-0 pr-1.5">{renderActionsMenu?.()}</div>
          </div>
                    ...
                    groupedActivities.map((item, index) => (
                      isActivityGroup(item) ? (
                        <ActivityGroupRow
                          ...
                          onExpandedGroupsChange={setExpandedActivityGroups}
      ...
      {!hasActivities && isThinking && <ThinkingPlaceholder />}
  ...
  // Re-render when response object changes
