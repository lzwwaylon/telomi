...
// Used by MarkdownView's anchor click handler (via resolveMarkdownLinkTarget)
// to route file links to onFileClick.
const FILE_PATH_TARGET_REGEX = new RegExp(`\\.(?:${FILE_EXTENSIONS_PATTERN})$`, 'i')

/** Strip navigation suffixes and decode only the path, leaving encoded filename characters intact. */
export function parseFilePathTarget(target: string): string | null {
  const rawPath = target.trim().split('#', 1)[0]!.replace(/:\d+(?::\d+)?$/, '')
  if (!rawPath || rawPath.includes('?')) return null
  let path: string
  try {
    path = decodeURIComponent(rawPath)
  } catch {
    // A literal percent is valid in a filename; malformed URL escapes must not throw during rendering.
    path = rawPath
  }
  if (/^[a-z][a-z\d+.-]*:|^\/\/|^(?:\/|\.\/)?api(?:\/|$)|[\u0000-\u001f\u007f]/i.test(path)) return null
...
function findMarkdownLinkRanges(text: string): CodeRange[] {
  ...
  const indexedCitationRegex = /\[\[\d+\]\]\([^)]*\)/g
  let match
  while ((match = indexedCitationRegex.exec(text)) !== null) {
    ranges.push({ start: match.index, end: match.index + match[0].length })
  }
...
export function detectLinks(text: string): DetectedLink[] {
  ...
  for (const match of urlMatches) {
    // linkify-it's fuzzy mode reads a bare `note.md` as the Moldovan domain, and step 2 below
    // yields to whatever matched here, so `.py`, `.rs`, `.sh` and `.md` filenames would render
    // as links to foreign hosts. A schemeless match carrying no path is a filename to us.
    if (!match.schema && !match.text.includes('/') && FILE_PATH_TARGET_REGEX.test(match.text)) continue
...
export function isFilePathTarget(target: string): boolean {
  return parseFilePathTarget(target) !== null
}
