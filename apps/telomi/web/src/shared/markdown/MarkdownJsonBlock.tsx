 ...
 * - Uses Telomi themes (transparent background and CSS variable fonts)
...
function isDarkMode(): boolean {
  ...
  return isCurrentPaperThemeDark()
}
...
export function MarkdownJsonBlock({ code, className }: MarkdownJsonBlockProps) {
  ...
  const jsonTheme = dark ? telomiDarkTheme : telomiLightTheme
      ...
      <div className={cn('relative group rounded-[8px] overflow-hidden border bg-muted/30', className)}>
        ...
        <div className="flex items-center justify-between px-3 py-1.5 bg-muted/50 border-b text-xs">
          ...
          <button
            ...
            aria-label={copied ? uiText("markdown.markdownjsonblock.jsonCopied") : uiText("markdown.markdownjsonblock.copyJson")}
          >
            {copied ? (
