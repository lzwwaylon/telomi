...
升级进程在停止 Telomi 之后意外崩溃时同样如此：退出前它会恢复之前的版本并重新
启动。
...
几秒，再以同一版本启动。如果自最新快照以来没有任何变化，它不停止服务、也不建立
快照，因为最新快照已经包含当前数据。判断依据是记录在每个快照 `snapshot.json` 中的
指纹，涵盖 Goal（Wiki、报告、附件、对话）、设置、连接、凭据和 User Memory 的内容。
不经用户操作、运行时自行变化的状态不计入：托管浏览器、服务日志、连接检测结果、保留策略
会清理的评估记录、连接同步的模型目录，以及自动刷新的登录令牌。命令保留最近 10 个升级前快照和最近 7 个
`--snapshot-only` 快照。
...
（`GET /api/runtime/idle`）。有 Goal 正在工作、有 Activity 在运行或排队、定时
调研即将开始，或你正在使用 Telomi 浏览器（登录来源，或接管 Browser Session）时，它输出原因、不做任何改动并以 `0` 退出。连续跳过满 24 小时后，
...
格式版本 3 改变了长期记忆本身：旧版本让所有 Goal 都能检索到的对话记忆，改为只属于
说出它的那个 Goal，升级后首次启动时会在记忆数据库中改写这些记忆。格式版本 4 删除
旧版本在记忆中保存的已确认 Topic Plan 副本，Topic Plan 本身不变。使用外部数据库时，
回到更早的格式需要连同数据目录一起还原它。
