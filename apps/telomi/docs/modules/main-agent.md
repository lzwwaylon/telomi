...
- Telomi 0.0.1 tagged every user message global. Data format version 3 records that those automatic tags must go; because Hindsight is not running while the data directory is migrated, the server removes them once memory answers (at startup, or before the first Memory page request if memory came up later), so no Episode the user makes global is ever stripped.
