...
Telomi 生成的 Goal、报告、Wiki、设置和已保存的凭据默认存放在 `apps/telomi/data`；设置 `TELOMI_DATA_DIR` 可改用其他目录。长期记忆存放在单独的本地 PostgreSQL（pg0）数据库中。需要备份的内容见[升级、备份与恢复](apps/telomi/docs/upgrading.zh-CN.md)。
