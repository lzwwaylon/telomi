## Problem and change / 问题与修改

<!-- What problem does this solve, and what happens after the change? Link a public Issue if applicable.
解决什么问题，修改后的行为是什么？适用时关联公开 Issue。 -->

## Verification / 验证

<!-- Reproduction steps, commands, results and tested OS/browser. State anything not tested.
提供复现步骤、验证命令、结果及系统/浏览器；说明未验证的部分。 -->

## Compatibility / 兼容性

<!-- Config, data format, dependency or upgrade impact; backup/recovery requirements. Write N/A if none.
配置、数据格式、依赖或升级影响，以及备份/恢复要求；无影响时写 N/A。 -->

- [ ] I read CONTRIBUTING.md and checked existing Issues/PRs. / 已阅读贡献指南并检索已有 Issue/PR。
- [ ] This PR covers one focused change and includes applicable verification. / 变更范围集中，并完成适用验证。
- [ ] Relevant documentation is updated, or no update is needed. / 已更新相关文档，或无需更新。
- [ ] Descriptions and evidence contain no credentials or private user data. / 描述及证据不含凭据或私人用户数据。
